...
namespace GT {
    ...
    namespace NET {
        ...
        public:
			...
			void InitLogAndCfgSrvice_(std::string cfg_path);
			bool Initialize(std::string cfg_ath);
			void GTStartService();
			void StopService();

			void RegisterServerCallBack(IO_EVENT_TYPE type, CallBackFunc call_back);
			void UnRegisterServerCallBack(IO_EVENT_TYPE);
			void DispatchEvent(IO_EVENT_TYPE type, SOCKETCONTEXT_SHAREPTR completion_key, IO_BUFFER_PTR io_ptr, long len);
		...
		private:
			std::function<void(IO_EVENT_TYPE, SOCKETCONTEXT_SHAREPTR, IO_BUFFER_PTR, long)> dispatch_event_func_;
